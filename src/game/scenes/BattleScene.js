...
export default class BattleScene extends Phaser.Scene {
    ...
    startGame(config, mapKey) {
        if (!mapKey) {
            ...
            // [Modified] 초기 카메라 설정: 맵 전체 높이를 UI 상단 위까지의 영역에 맞춤
            this.updateCameraBounds(this.scale.width, this.scale.height); 
            this.fitCameraToMap(); 
        ...
        // [Modified] 초기 setup 단계에서는 전체 맵을 보여주기 위해 follow 끔
    ...
    // [New] 맵 전체 높이를 UI 상단까지의 영역에 맞춰 공백 없이 표시하는 함수
    fitCameraToMap() {
        ...
        const shopHeight = isMobile ? 60 : 80; 
        const totalBottomUIHeight = footerHeight + shopHeight;

        // UI를 제외한 '보여야 할' 높이
        const availableHeight = height - totalBottomUIHeight;

        // 줌 비율 계산: 
        // 1. 너비 기준: 화면 너비 / 맵 너비
        // 2. 높이 기준: 가용 높이 / 맵 높이
        // 둘 중 큰 값을 선택해야 빈 공간 없이(Cover) 채워짐.
        ...
        // Bounds를 맵 크기로 강제 고정 (공백 방지)
        this.cameras.main.setBounds(0, 0, this.mapWidth, this.mapHeight);

        // 맵 바닥을 UI 상단에 맞추기 위한 Y좌표 계산
        // 맵의 전체 높이가 zoom된 상태에서 availableHeight 영역에 들어와야 함.
        // Phaser 카메라는 scrollY가 Viewport의 Top-Left 기준.
        // 맵의 바닥(mapHeight)이 화면의 (height - uiHeight) 지점에 오도록 설정.
        this.cameras.main.scrollY = this.mapHeight - (availableHeight / targetZoom);
    }
    ...
    updateCameraBounds(w, h) { 
        ...
        // [Modified] 공백 방지를 위해 패딩 없이 맵 크기에 딱 맞춤
        this.cameras.main.setBounds(0, 0, this.mapWidth, this.mapHeight);
    }
    ...
    handleStartBattle() {
        ...
        // [Modified] 전투 시작 시 카메라 연출: 화면 전체에 꽉 차게 줌/팬
        if (this.mapWidth && this.mapHeight) {
            const { width, height } = this.scale;
            
            // 화면 전체를 기준으로 줌 레벨 재계산 (UI 사라진 후)
            const zoomX = width / this.mapWidth;
            const zoomY = height / this.mapHeight;
            const targetZoom = Math.max(zoomX, zoomY);
            
            // 애니메이션: 줌과 동시에 화면 중앙(또는 하단 정렬)으로 이동
            this.tweens.add({
                targets: this.cameras.main,
                zoom: targetZoom,
                scrollY: this.mapHeight - (height / targetZoom), // 하단 정렬 유지
                scrollX: (this.mapWidth - (width / targetZoom)) / 2, // 수평 중앙 정렬
                duration: 500,
                ease: 'Power2',
                onComplete: () => {
